...
class Dpm3Det(nn.Module):
    """
    Implement DiffusionDet
    """

    def __init__(self, cfg, 
        statistics_dir = './samplers/',
        noise_schedule = 'logSNR',
        steps=10,
        t_start=None,
        t_end=None,
    ...
    @torch.no_grad()
    def dpm3_sample(self, batched_inputs, backbone_feats, images_whwh, images, clip_denoised=True, do_postprocess=True, 
                      unconditional_conditioning=None,use_corrector=False,  half=False):
        ...
        for step in range(1, steps + 1):
            time_cond = torch.full((batch,), step, device=self.device, dtype=torch.long)
            self_cond = x_start if self.self_condition else None
            cached_x, cached_model_output, cached_time, cached_index = [], [], [], []
            ...
            cached_x.append(img)
            cached_model_output.append(pred_noise)
            cached_time.append(timesteps[step - 1])
            cached_index.append(indexes[step - 1])
            if use_corrector and (timesteps[step - 1] > 0.5 or not half):
                step_c_order = step_p_order + c_pseudo
            ...
            img = self.multistep_predictor_update(
                cached_x, cached_model_output, cached_time, cached_index, t, i_t, order=step_p_order, pseudo=p_pseudo
            )
    ...
    def q_sample(self, x_start, t, noise=None):
        if noise is None:
            noise = torch.randn_like(x_start)   #zhengtaifenbu
