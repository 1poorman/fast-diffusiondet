...
| 假设 | 内容 | 当前判定 | 依据 |
|---|---|---|---|
| H1 | NFE≤4 时 2 阶+ 求解器 AP ≥ DDIM +0.5 | ✅ **支持**（M2/M8） | DPP@4 +1.31；终版 DPP@2=70.36 全局最优 |
| H2 | PFGM++ 使 NFE=2 ≈ DDIM@8 | ⬜ 待测（M5） | — |
| H3 | EMS 真统计量仅在 NFE≤5 有可见收益 | ❌ **否定**（M3/K256） | 仅 NFE=2 +0.7；NFE≥3 系统性退化 |
| H4 | EDM 主要提升上界而非低 NFE 效率 | ❌ **否定**（M4/M5） | box 域不收敛（AP<16 vs 67）；c_noise 尺度为已确认缺陷 |
| **H0** | head forward 是延迟主项、NFE 与延迟线性相关 | ✅ **支持** | §2：边际 71.5 ms/step，占比 66% |


---

## 10. 训练时长延长实验（2026-09-26，响应"loss 未收敛是否加训"）

背景：fig2 显示 PLS/WHEAT 末段 loss 仍在下降。量化检查：PLS 末段斜率
−1.2/1000it 但 AP=98.2 已饱和（不必加训）；**WHEAT 末段斜率 −4.8/1000it
强劲下降（明显欠训练）→ 延长**；SDD 同步延长对照。

配置：MAX_ITER 1400/1750 → 4000（STEPS 3200 = 80%，EVAL_PERIOD 1000 监控拐点）。
数据：`results/raw/m8_{sdd,wheat}_long.csv`。

| 数据集 | 短训 AP（正式 3 seed） | 长训 AP | 判定 |
|---|---|---|---|
| SDD | 67.16±0.13 | **70.27±0.14（ddim@1）/ 70.36±0.13（DPP@2）** | **+3.1 显著收益** |
| WHEAT | 24.16±0.13 | 23.96±0.46 | 无收益（噪声内），16 epoch 已饱和 |
| PLS | 98.26±0.01 | 未跑（天花板无空间） | 不适用 |

SDD 长训 inline AP 轨迹：55.6@1000 → 62.4@2000 → 64.0@3000（LR 衰减前）→
**70.2@4000（衰减后跳涨）**——SDD 短训的瓶颈是"LR 衰减时模型未到平台"，
衰减后 800 iter 精修贡献了最后 ~6 AP。WHEAT 相反：val AP 不随 loss 下降而升
（小数据集 695 图的过拟合分界），16 epoch 即饱和，延长无益。

**新增全局最优：SDD D4-long + DPP@NFE2 = 70.36±0.13**（此前 67.11）。
**修订推荐配置链**：
```
训练：configs/lab/sdd.res50.bs8.long.yaml（bs8/LR1e-4/4000it/STEPS 3200）
推理：DPP@NFE2 = 70.36（精度优先）/ DDIM@NFE1 = 70.27（延迟优先）
```
注意：延长训练后 DPP@2 与 DDIM@1 差距缩至 +0.09（std 0.13，噪声内）——
训练越充分采样器差异越小，进一步支持"训练策略 > 采样器工程"。
