...
# ============================================================================
# 移植自 DPM-Solver-v3 官方实现（MIT License, Copyright (c) 2023 TSAIL group）
#   源: ../DPM-Solver-v3-main/dpm_solver_v3.py
# 相对官方版本的**唯一**改动：
#   把所有硬编码的 `.cuda()` 换成 `.to(self.device)`，使其可在任意 device 上工作。
# 其余算法与系数计算逐行保持原样，便于与官方结果对拍。
# ============================================================================
...
class DPM_Solver_v3:
    def __init__(
        ...
        # NOTE(fast-diffusiondet): 官方这里是硬编码的 ``[:, None, None, None]``（假设图像
        # (N+1, C, H, W)）。本项目 l/s/b 的形状是可配置的（如 (N+1,1,1) 各向同性或
        # (N+1,1,4) 逐坐标），因此按 l 的维数动态补轴；对官方的 4 维情形行为完全不变。
        ts = noise_schedule.marginal_lambda(
            self.get_time_steps("logSNR", t_T, t_0, self.statistics_steps, "cpu")
        ).numpy()
        ts = ts.reshape((-1,) + (1,) * (l.ndim - 1))
        ...
        self.l = torch.from_numpy(l).to(self.device)
        self.s = torch.from_numpy(s).to(self.device)
        self.b = torch.from_numpy(b).to(self.device)
        self.L = torch.from_numpy(L).to(self.device)
        self.S = torch.from_numpy(S).to(self.device)
        self.I = torch.from_numpy(I).to(self.device)
        self.B = torch.from_numpy(B).to(self.device)
    ...
    def multistep_predictor_update(self, x_lst, eps_lst, time_lst, index_lst, t, i_t, order=1, pseudo=False):
        ...
        time_0n = torch.FloatTensor(index_list(time_lst, indexes)).to(self.device)
