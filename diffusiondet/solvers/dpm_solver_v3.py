# fast-diffusiondet M1: DPM-Solver-v3 移植（来自 thu-ml/DPM-Solver-v3）
# ========================================
# 移植自 https://github.com/thu-ml/DPM-Solver-v3 （dpm_solver_v3.py，原样保留算法），
# 适配性修改：
#   1. 去除 .cuda() 硬编码 -> 统一使用构造参数 device；
#   2. statistics_dir 可为 None：degenerated=True 时合成 l=1, s=0, b=0
#      （等价于 DPM-Solver++，M2 即可用，无需 EMS 统计量）；
#   3. 关闭默认 print（verbose 参数控制）；
#   4. 末尾新增 dpm_v3_sample()：与 detector/denoise_fn 对接的封装。
# 注意：我们的模型是离散时间模型，连续时间 t 会在 dpm_v3_sample 内部
# snap 到最近离散索引（round 后查表），时间嵌入输入与 DDIM 同域。
...
class DPM_Solver_v3:
    def __init__(
        ...
        if statistics_dir is not None:
            ...
            # 无统计量：仅支持 degenerated 模式，合成 l=1, s=0, b=0
            assert degenerated, "statistics_dir=None requires degenerated=True"
            l = np.ones((statistics_steps + 1,), dtype=np.float64)
            s = np.zeros((statistics_steps + 1,), dtype=np.float64)
            b = np.zeros((statistics_steps + 1,), dtype=np.float64)
        if degenerated:
            l = np.ones_like(l)
            s = np.zeros_like(s)
            b = np.zeros_like(b)
        ...
        # 统一 cast 成 float32：避免多步更新中 x 被提升为 float64，
        # 传回 float32 的 head 时 dtype 不匹配（原版依赖 npz 为 float32 的隐含约定）
        self.l = torch.from_numpy(l).to(device=self.device, dtype=torch.float32)
        self.s = torch.from_numpy(s).to(device=self.device, dtype=torch.float32)
        self.b = torch.from_numpy(b).to(device=self.device, dtype=torch.float32)
        self.L = torch.from_numpy(L).to(device=self.device, dtype=torch.float32)
        self.S = torch.from_numpy(S).to(device=self.device, dtype=torch.float32)
        self.I = torch.from_numpy(I).to(device=self.device, dtype=torch.float32)
        self.B = torch.from_numpy(B).to(device=self.device, dtype=torch.float32)
        self.C = torch.from_numpy(C).to(device=self.device, dtype=torch.float32)
    ...
    def multistep_predictor_update(self, x_lst, eps_lst, time_lst, index_lst, t, i_t, order=1, pseudo=False):
        ...
        time_0n = torch.stack([t.detach().reshape(()) for t in index_list(time_lst, indexes)]).to(self.device)
...
# =============================================================
# fast-diffusiondet 封装：与 DiffusionDet detector / DenoiseFn 对接
# =============================================================

@torch.no_grad()
def dpm_v3_sample(detector, denoise_fn, batch, shape, batched_inputs, images,
                  do_postprocess=True, noise=None):
    """DPM-Solver-v3 采样入口（蓝图 §3.3 的调用协议）。

    * NoiseScheduleVP(discrete)：betas 与 detector 的 cosine schedule 同源；
    * model_wrapper(model_type="x_start")：DiffusionDet 是 x0 参数化；
    * 连续时间 t_input=(t-1/N)*1000 会被 snap 到最近离散索引（离散时间模型）；
    * steps = sampling_timesteps（SAMPLE_STEP），保证 NFE 语义与 DDIM/Heun 一致；
    * use_corrector=True, c_pseudo=True, p_pseudo=False, lower_order_final=True
      为蓝图 §3.3 钦定的默认组合。
    """
    ...
    def model(x, t_input, cond=None):
        ...
        pred_noise, x0 = denoise_fn(x, t)
        step_records.append((denoise_fn.last_outputs_class, denoise_fn.last_outputs_coord))
        return x0

    model_fn = model_wrapper(model, ns, model_type="x_start")
    solver = DPM_Solver_v3(
        statistics_dir=detector.dpm_stats_dir if detector.dpm_stats_dir else None,
        noise_schedule=ns,
        steps=detector.sampling_timesteps,
        skip_type=detector.dpm_skip_type,
        degenerated=detector.dpm_degenerated,
        device=device,
    )
    ...
    # DDIM 语义的 ensemble：排除最后一个（终点投影）步
    ensemble_score, ensemble_label, ensemble_coord = [], [], []
    if use_ens:
        for outputs_class, outputs_coord in step_records[:-1]:
